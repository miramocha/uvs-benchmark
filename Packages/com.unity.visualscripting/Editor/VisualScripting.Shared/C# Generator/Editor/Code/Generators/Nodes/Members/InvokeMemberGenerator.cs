...
namespace Unity.VisualScripting.Community.CSharp
{
    ...
    public sealed class InvokeMemberGenerator : LocalVariableGenerator
    {
        ...
        protected override void GenerateValueInternal(ValueInput input, ControlGenerationData data, CodeWriter writer)
        {
            ...
            else if (input.hasDefaultValue)
            {
                ...
                if (input == Unit.target && unit.defaultValues[Unit.target.key] == null && (isGameObject || isComponent))
                {
                    ...
                    if (isComponent)
                    {
                        writer.Write(input.GetComponent(writer, GetSourceType(input, data, writer), input.type, true, true));
                    }
                }
            ...
            else
            {
                ...
                {
                    if (Unit.member.methodInfo.GetParameters()[Array.IndexOf(Unit.inputParameters, input)].IsDefined(typeof(ParamArrayAttribute), true))
                    {
                        return;
                    }
                    else if (Unit.member.methodInfo.GetParameters()[Array.IndexOf(Unit.inputParameters, input)].IsOptional)
                    {
                        return;
                    }
                }
                ...
                {
                    if (Unit.member.constructorInfo.GetParameters()[Array.IndexOf(Unit.inputParameters, input)].IsDefined(typeof(ParamArrayAttribute), true))
                    {
                        return;
                    }
                    else if (Unit.member.constructorInfo.GetParameters()[Array.IndexOf(Unit.inputParameters, input)].IsOptional)
                    {
                        return;
                    }
                }
        ...
        protected override void GenerateValueInternal(ValueOutput output, ControlGenerationData data, CodeWriter writer)
        {
            ...
            {
                if (!Unit.enter.hasValidConnection && Unit.outputParameters.Length > 0)
                {
            ...
            if (Array.IndexOf(Unit.outputParameters, output) != -1)
            {
        ...
        private void GenerateArguments(CodeWriter writer, ControlGenerationData data)
        {
            ...
            for (int i = startIndex; i < parameters.Length; i++)
            {
                ...
                var input = (uint)i < (uint)Unit.inputParameters.Length ? Unit.inputParameters[i] : null;
                ...
                if (param.HasOutModifier())
                {
                    ...
                    var outValue = (uint)i < (uint)Unit.outputParameters.Length ? Unit.outputParameters[i] : null;
                    if (outValue != null && !outputNames.ContainsKey(outValue))
                    {
