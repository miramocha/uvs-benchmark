...
namespace Unity.VisualScripting
{
    ...
    public abstract class Unit : GraphElement<FlowGraph>, IUnit
    {
        ...
        protected Unit() : base()
        {
            ...
            defaultValues = new Dictionary<string, object>(StringComparer.Ordinal);
        }
        ...
        {
            EnsureUniqueOutput(key);
            var port = new ValueOutput(key, type, (flow) => new ParameterValue(getValue(flow)));
            valueOutputs.Add(port);
            return port;
        }

        protected ValueOutput ValueOutput(Type type, string key, Func<Flow, ParameterValue> getValue)
        {
            EnsureUniqueOutput(key);
        ...
        public string GetElementStackTrace(GraphReference reference, Unit unit, string pathPrefix = "/")
        {
            string BuildPrefix(GraphReference nodePath)
            {
                ...
                if (nodePath.isRoot)
                {
                    if (!(reference.rootObject is IEventMachine) && reference.rootObject is UnityEngine.Object asset)
                    {
                        return asset.name;
                    }
                    ...
                    {
                        if (eventMachine.nest.source == GraphSource.Macro && eventMachine.nest.macro is UnityEngine.Object @object)
                        {
                            return @object.name;
                        }
                        ...
                        return $"Unnamed {typeName} at Index({index})";
                    }
                }

                if (nodePath.IsWithin<INesterUnit>())
                {
                    var parent = nodePath.GetParent<INesterUnit>();
                    return GetUnitName(parent);
                }
            ...
            string cleanPathNames = pathNames.TrimEnd();
            if (cleanPathNames.EndsWith("/"))
            {
                cleanPathNames = cleanPathNames.Substring(0, cleanPathNames.Length - 1);
            }

            string rootName = reference.rootObject != null ? $"{reference.rootObject.name}" : "";

            string sceneName = reference.scene.HasValue ? reference.scene.Value.name :
#if UNITY_EDITOR
            AssetDatabase.GetAssetPath(reference.rootObject);
#else
            "UnknownSceneOrAsset";
#endif

            string fakePath = $"{sceneName}/{rootName}/{cleanPathNames}";
        ...
        public static string GetUnitName(IUnit unit)
        {
            return (unit as Unit)?.GetUnitName() ?? "UnknownUnit";
        }

        protected string UnitTitle(Type unitType, bool @short, bool includeStatus)
        {
            ...
            return title != null ? $"{title}{suffix}" : $"{unitType.CSharpName()}{suffix}";
        }
    }
}
