            ...
            try
            {
                ...
                foreach (var handler in wrapper.handlers)
                {
                    ...
                    if (!string.IsNullOrEmpty(handler.scenePath))
                    {
...
#if UNITY_6000_4_OR_NEWER
                            var goId = EntityId.FromULong((ulong)int.Parse(split[0]));
#else
                            var goId = int.Parse(split[0]);
#endif

#if UNITY_6000_3_OR_NEWER
                            var go = EditorUtility.EntityIdToObject(goId) as GameObject;
#else
...
#if UNITY_6000_4_OR_NEWER
                                    .FirstOrDefault(c => c != null && c.GetEntityId() == EntityId.FromULong((ulong)int.Parse(split[1])));
#else
                        ...
                        {
#if UNITY_6000_4_OR_NEWER
                            var instanceID = EntityId.FromULong((ulong)int.Parse(handler.scenePath));
#else
                            var instanceID = int.Parse(handler.scenePath);
#endif

#if UNITY_6000_3_OR_NEWER
                            target = EditorUtility.EntityIdToObject(instanceID) as GameObject;
#else
                    ...
                    foreach (var pair in handler.sceneObjectValues)
                    {
...
#if UNITY_6000_4_OR_NEWER
                                    .FirstOrDefault(c => c.GetEntityId() == EntityId.FromULong((ulong)int.Parse(split[1])));
#else
                        ...
                        {
#if UNITY_6000_4_OR_NEWER
                            var instanceID = EntityId.FromULong((ulong)int.Parse(pair.value));
#else
                            var instanceID = int.Parse(pair.value);
#endif

#if UNITY_6000_3_OR_NEWER
                            sceneValue = EditorUtility.EntityIdToObject(instanceID);
#else
