    ...
    public static partial class HUMValue
    {
        ...
        public static object Deserialized(this string str)
        {
            ...
            if (str.Contains("[Type:GameObject"))
            {
                ...
                var asset = AssetDatabase.LoadAssetAtPath<GameObject>(AssetDatabase.GetAssetPath((EntityId)parsedInt));
                return asset;
#elif UNITY_6000_4_OR_NEWER
                var parsedUlong = ulong.Parse(str.Replace("[Type:GameObject]", string.Empty));
                var asset = AssetDatabase.LoadAssetAtPath<GameObject>(AssetDatabase.GetAssetPath(EntityId.FromULong(parsedUlong)));
                return asset;
#else
                var parsedInt = int.Parse(str.Replace("[Type:GameObject]", string.Empty));
