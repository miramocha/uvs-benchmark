...
namespace Unity.VisualScripting
{
    public abstract class ReflectedCloner : Cloner<object>
    {
        ...
        private readonly Dictionary<Type, OptimizedAccessorBase[]> optimizedAccessors = new Dictionary<Type, OptimizedAccessorBase[]>();

        private OptimizedAccessorBase[] GetOptimizedAccessors(Type type)
        {
            if (!optimizedAccessors.ContainsKey(type))
            {
                var list = new List<OptimizedAccessorBase>();
