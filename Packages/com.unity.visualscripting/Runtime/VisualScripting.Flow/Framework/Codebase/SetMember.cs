...
namespace Unity.VisualScripting
{
    ...
    public sealed class SetMember : MemberUnit
    {
        ...
        private ControlOutput Assign(Flow flow)
        {
            ...
            switch (strategy)
            {
                case AccessStrategy.Static: flow.SetValue(output, member.Set(target, flow.GetValueData(input))); break;
                case AccessStrategy.Instance: flow.SetValue(output, member.Set(target, flow.GetValueData(input))); break;
                case AccessStrategy.Reference: flow.SetValue(output, member.SetRef(ref target, flow.GetValueData(input))); break;
            }

            if (requiresTarget && chainable)
            {
